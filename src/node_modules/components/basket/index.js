import React from 'react'
import {connect} from "react-redux";
import {getBasketPhonesWithCount, getTotalBasketPrice} from "selectors";
import * as R from 'ramda'
import {removePhoneFromBasket,basketCheckout,cleanBasket} from 'actions'
import {Link} from "react-router-dom";
let Basket = ({phones,totalPrice,removePhoneFromBasket,basketCheckout,cleanBasket})=>{
    const isBasketEmpty = R.isEmpty(phones) ;
    const renderContent = () => {
        return(
            <div>
                {isBasketEmpty && <div>Your shopping cart is empty</div>}
                <div className='table-responsive'>
                    <table className='table-bordered table-striped table-condensed cf'>
                        <tbody>
                        {phones.map((phone,index)=>{
                           return( <tr className='item-checout'
                                key={index}>
                               <td className='first-column-checkout' >
                                   <img className='img-thumbnail' src={phone.image} alt={phone.name}/>
                               </td>
                               <td>
                                   {phone.name}
                               </td>
                               <td>
                                   ${phone.price}
                               </td>
                               <td>
                                   Count:{phone.count}
                               </td>
                               <td>
                                   <span
                                       className='delete-cart'
                                   onClick={()=> removePhoneFromBasket(phone.id)}>Delete</span>
                               </td>
                            </tr>)
                        })}
                        </tbody>
                    </table>
                </div>
                {R.not(isBasketEmpty) &&
                <div className={'row'}>
                    <div className={'pull-right total-user-chekout'}>
                        <b>Total:</b>
                        ${totalPrice}
                    </div>
                </div>}
            </div>
        )
    }
    const renderSidebar = () => {
     return(
         <div>
             <Link to={'/'} className={'btn btn-info'}>

                 <span className={'glyphicon glyphicon-info-sign'}></span>
                 <span>Continue shopping!</span>
             </Link>
             {R.not(isBasketEmpty) &&
             <div>
                 <button onClick={cleanBasket} className={'btn btn-danger'}>
                     <span className={'glyphicon-trash '}></span>Clean Trash
                 </button>
                 <button className={'btn btn-success'} onClick={() => basketCheckout(phones)}>
                     <span className={'glyphicon-envelope'}></span>
                 </button>
                 }
             </div>
             }
         </div>
     )
    }

    return(
        <div className='viev-container'>
            <div className='container'>
                <div className='row'>
                    <div className='col-md-9'>
                        {renderContent()}
                    </div>
                    <div className='col-md-3 btn-user-checkout'>
                        {renderSidebar()}
                    </div>
                </div>
            </div>
        </div>
    )
                }
let mapStateToProps = state =>{
    return {
        phones : getBasketPhonesWithCount(state),
        totalPrice : getTotalBasketPrice(state)
    }
}
let mapDispatchToProps = {
    removePhoneFromBasket,
    basketCheckout,
    cleanBasket
}
export default connect(mapStateToProps,mapDispatchToProps)(Basket)